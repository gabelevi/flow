...
type entry_point = (
  SharedMem_js.config * Options.t * Hh_logger.Level.t * FlowEventLogger.logging_context * string array,
  in_channel,
  daemon_msg
) Daemon.entry
...
: entry_point =
  Daemon.register_entry_point
    (new_entry_point ())
    (fun (shared_mem_config, options, hh_logger_level, logging_context, argv) (ic, waiting_channel) ->
